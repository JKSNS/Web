...
# 6. Install PHP 7.4 & extensions
echo "Installing PHP 7.4 and required modules..."
if [ "$pm" == "apt-get" ]; then
    # Explicitly install PHP 7.4 and its Apache module plus common extensions
    eval "$install_cmd php7.4 libapache2-mod-php7.4 php7.4-cli php7.4-common php7.4-mbstring php7.4-gd php7.4-intl php7.4-xml php7.4-mysql php7.4-zip php7.4-curl php7.4-xmlrpc"
else
    # For Fedora/RHEL-based systems, adjust package names as necessary.
    # Example using dnf (ensure that the appropriate PHP module stream is enabled):
    if [ "$pm" == "dnf" ]; then
        sudo dnf module reset php -y
        sudo dnf module enable php:7.4 -y
        eval "$install_cmd php php-cli php-common php-mbstring php-gd php-intl php-xml php-mysqlnd php-zip php-curl php-xmlrpc"
    else
        # For yum or zypper, you may need to adjust package names or use repositories that provide PHP 7.4.
        eval "$install_cmd php php-cli php-common php-mbstring php-gd php-intl php-xml php-mysqlnd php-zip php-curl php-xmlrpc"
    fi
fi
